...
--]=====================================================]
-- FIX: COMBAT_TEXT_LOW_HEALTH_THRESHOLD / COMBAT_TEXT_LOW_MANA_THRESHOLD are
-- Blizzard globals that only exist once Blizzard_CombatText has loaded. If
-- it hasn't (e.g. Blizzard's own Floating Combat Text is disabled), they are
-- nil and any comparison against them errors out. Fall back to Blizzard's
-- own default of 20% so xCT+ doesn't depend on that module being loaded.
local FALLBACK_LOW_HEALTH_THRESHOLD = COMBAT_TEXT_LOW_HEALTH_THRESHOLD or 0.2
local FALLBACK_LOW_MANA_THRESHOLD = COMBAT_TEXT_LOW_MANA_THRESHOLD or 0.2
...
x.events = {
  ["UNIT_HEALTH"] = function()
      ...
      if ShowLowResources() and healthPct <= FALLBACK_LOW_HEALTH_THRESHOLD then
  ...
  ["UNIT_POWER_UPDATE"] = function(unit, powerType)
      ...
      if ShowLowResources() and manaPct <= FALLBACK_LOW_MANA_THRESHOLD then
